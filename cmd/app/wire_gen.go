...
func wireApp(configuration *config.Configuration, lumberjackLogger *lumberjack.Logger, zapLogger *zap.Logger) (*App, func(), error) {
	...
	compService := 	service.NewCompService(compRepo, transaction)
	...
	compHanler := app.NewCompHandler(zapLogger,jwtService,compService)
	...
	engine := router.NewRouter(configuration, jwtAuth, recovery, cors, limiter, authHandler, uploadHandler, compHanler)
